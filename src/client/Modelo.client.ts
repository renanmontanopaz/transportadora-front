...
export class ModeloClient {
    ...
    public async excluir(modelo: Modelo) : Promise<void> {
        try {
            return (await this.axiosClient.put(`/${modelo.id}`)).data
        }
        catch(error:any) {
            return Promise.reject(error.response)
        }
    ...
    public async findByMarca(idMarca: number) : Promise<Modelo> {
        try {
            return (await this.axiosClient.get<Modelo>(`/${idMarca}`)).data
        }
        catch(error:any) {
            return Promise.reject(error.response)
        }
